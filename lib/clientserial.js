(function(master){
	...
	dp.onReceive = function(data) {
		...
		if(seqId === 0) { //Event
			if(cmd === CMD.ONRECEIVE) {
				var connectionId = data[0].connectionId,
					data = data[0].data;
	...
	dp.connect = function(callback) {
		callback = callback || EmptyFunction;
		var that = this;
		this.ws = new WebSocket('ws://127.0.0.1:8889/json');
		this.ws.addEventListener('open', function() {
			that.sendCommand(CMD.HELLO, {ver: that.version}, function(e, data) {
				if(e)
					return callback(e);
				that.isConnected = true;
				that.serverVersion = data.version;
	...
	var SerialConnection = global.Connection = function(siface) {
		this.siface = siface;
		this.connectionId = -1;
	}
	...
	dp.connect = function(path, params, callback) {
		var that = this;
		this.siface.sendCommand(CMD.CONNECT, path, params, function(e, ret) {
			if(e)
				return callback(e);
			that.connectionId = ret.connectionId;
	...
	dp.disconnect = function(callback) {
		this.siface.sendCommand(CMD.DISCONNECT, this.connectionId, callback)
		this.connectionId = -1;
	}

	dp.write = function(data, callback) {
		this.siface.sendCommand(CMD.WRITE, this.connectionId, data, callback)
	}
