CTRL.TabbedPane = function()
{
    ...
    /** @type {!Array.<!CTRL.TabbedPaneTab>} */
    this._tabs = [];
    /** @type {!Array.<!CTRL.TabbedPaneTab>} */
    this._tabsHistory = [];
    /** @type {!Object.<string, !CTRL.TabbedPaneTab>} */
    this._tabsById = {};
    this._currentTabLocked = false;

    this._dropDownButton = this._createDropDownButton();
    CTRL.zoomManager.addEventListener(CTRL.ZoomManager.Events.ZoomChanged, this._zoomChanged, this);
}

CTRL.TabbedPane.EventTypes = {
    TabSelected: "TabSelected",
    TabClosed: "TabClosed",
    TabOrderChanged: "TabOrderChanged"
}

CTRL.TabbedPane.prototype = {
    /**
    ...
    /**
     * @return {!Array.<!CTRL.Widget>}
     */
    tabViews: function()
    {
        /**
         * @param {!CTRL.TabbedPaneTab} tab
         * @return {!CTRL.Widget}
         */
    ...
    /**
     * @param {!CTRL.TabbedPaneTabDelegate} delegate
     */
    ...
    appendTab: function(id, tabTitle, view, tabTooltip, userGesture, isCloseable, index)
    {
        ...
        var tab = new CTRL.TabbedPaneTab(this, id, tabTitle, isCloseable, view, tabTooltip);
    ...
    _innerCloseTab: function(id, userGesture)
    {
        ...
        this.dispatchEventToListeners(CTRL.TabbedPane.EventTypes.TabClosed, eventData);
        return true;
    },
    ...
    selectTab: function(id, userGesture)
    {
        ...
        var eventData = { tabId: id, view: tab.view, isUserGesture: userGesture };
        this.dispatchEventToListeners(CTRL.TabbedPane.EventTypes.TabSelected, eventData);
        return true;
    },
    ...
    {
        var constraints = CTRL.VBox.prototype.calculateConstraints.call(this);
    ...
    {
        CTRL.invokeOnceAfterBatchUpdate(this, this._innerUpdateTabElements);
    },
    ...
    _createDropDownButton: function()
    {
        ...
        this._dropDownMenu = new CTRL.DropDownMenu(dropDownContainer);
        this._dropDownMenu.addEventListener(CTRL.DropDownMenu.Events.ItemSelected, this._dropDownMenuItemSelected, this);
    ...
    /**
     * @param {!Array.<!CTRL.TabbedPaneTab>} tabsOrdered
     * @param {!Array.<!CTRL.TabbedPaneTab>} tabsHistory
     * @param {number} totalWidth
    ...
    _insertBefore: function(tab, index)
    {
        ...
        if (oldIndex < index)
            --index;
        this._tabs.splice(index, 0, tab);
        this.dispatchEventToListeners(CTRL.TabbedPane.EventTypes.TabOrderChanged, this._tabs);
    },
...
/**
 * @constructor
 * @param {!CTRL.TabbedPane} tabbedPane
 * @param {string} id
 * @param {string} title
 ...
 */
CTRL.TabbedPaneTab = function(tabbedPane, id, title, closeable, view, tooltip)
{
...
CTRL.TabbedPaneTab.prototype = {
    ...
    _createTabElement: function(measuring)
    {
        ...
        if (measuring) {
            ...
            if (this._tabbedPane._allowTabReorder)
                CTRL.installDragHandle(tabElement, this._startTabDragging.bind(this), this._tabDragging.bind(this), this._endTabDragging.bind(this), "-webkit-grabbing", "pointer");
        }
    ...
    _tabContextMenu: function(event)
    {
        ...
        if (this._closeable) {
            contextMenu.appendItem(CTRL.UIString.capitalize("Close"), close.bind(this));
            contextMenu.appendItem(CTRL.UIString.capitalize("Close ^others"), closeOthers.bind(this));
            contextMenu.appendItem(CTRL.UIString.capitalize("Close ^tabs to the ^right"), closeToTheRight.bind(this));
            contextMenu.appendItem(CTRL.UIString.capitalize("Close ^all"), closeAll.bind(this));
        }
        if (this._delegate)
 ...
 * @interface
 */
CTRL.TabbedPaneTabDelegate = function()
{
}

CTRL.TabbedPaneTabDelegate.prototype = {
    /**
     * @param {!CTRL.TabbedPane} tabbedPane
     * @param {!Array.<string>} ids
     */
    ...
    /**
     * @param {string} tabId
     * @param {!CTRL.ContextMenu} contextMenu
     */
 ...
 * @param {function(string, !CTRL.Widget)=} viewCallback
 */
CTRL.ExtensibleTabbedPaneController = function(tabbedPane, extensionPoint, viewCallback)
{
    this._tabbedPane = tabbedPane;
    this._extensionPoint = extensionPoint;
    this._viewCallback = viewCallback;
    this._tabOrders = {};
    /** @type {!Object.<string, !Promise.<?CTRL.Widget>>} */
    this._promiseForId = {};

    this._tabbedPane.addEventListener(CTRL.TabbedPane.EventTypes.TabSelected, this._tabSelected, this);
    /** @type {!Map.<string, ?CTRL.Widget>} */
    this._views = new Map();
    this._initialize();
}

CTRL.ExtensibleTabbedPaneController.prototype = {
    _initialize: function()
    {
        ...
        for (var i = 0; i < extensions.length; ++i) {
            ...
            var title = CTRL.UIString(descriptor["title"]);

            this._extensions.set(id, extensions[i]);
            this._tabbedPane.appendTab(id, title, new CTRL.Widget());
        }
    },
    ...
    _tabSelected: function(event)
    {
        ...
        /**
         * @this {CTRL.ExtensibleTabbedPaneController}
         * @param {?CTRL.Widget} view
         */
        function viewLoaded(view)
        {
            ...
            var shouldFocus = this._tabbedPane.visibleView.element.isSelfOrAncestor(CTRL.currentFocusElement());
            if (shouldFocus)
                view.focus();
        }
    },
    ...
    /**
     * @param {string} id
     * @return {!Promise.<?CTRL.Widget>}
     */
    viewForId: function(id)
    {
        if (this._views.has(id))
            return Promise.resolve(/** @type {?CTRL.Widget} */ (this._views.get(id)));
        if (!this._extensions.has(id))
            return Promise.resolve(/** @type {?CTRL.Widget} */ (null));
        if (this._promiseForId[id])
            return this._promiseForId[id];

        var promise = this._extensions.get(id).instancePromise();
        this._promiseForId[id] = /** @type {!Promise.<?CTRL.Widget>} */ (promise);
        ...
        {
            var view = /** @type {!CTRL.Widget} */ (object);
